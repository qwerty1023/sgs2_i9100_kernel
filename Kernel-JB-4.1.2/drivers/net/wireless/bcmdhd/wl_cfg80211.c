	...
	bool enabled, s32 timeout)
{
	...
	RETURN_EIO_IF_NOT_UP(wl);
	WL_DBG(("Enter\n"));
	if (wl->p2p_net == dev || _net_info == NULL || wl->vsdb_mode || !wl_get_drv_status(wl, CONNECTED, dev)) { 
		return err;
	}
...
static void wl_cfg80211_work_handler(struct work_struct * work)
{
	...
	if (wl->pm_enable_work_on) {
		wl->pm_enable_work_on = false;
	for_each_ndev(wl, iter, next) {
		if (!wl_get_drv_status(wl, CONNECTED, iter->ndev) || (wl_get_mode_by_netdev(wl, iter->ndev) != WL_MODE_BSS))
			continue;
		if (iter->ndev) {
